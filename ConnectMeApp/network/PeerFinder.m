...
- (void)startScan {
  ...
  NSLog(@"self ip address %@",self.localAddress);
  ...
  NSLog(@"Base address %@",self.baseAddress);
}
...
- (void)pingAddress{
  ...
  // NSLog(@"Ping address %@",address);
  [SimplePingHandler ping:address target:self sel:@selector(pingResult:)];
