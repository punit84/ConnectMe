...
@property (nonatomic) NSInteger filenameCounter;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
  ...
  MCConnection *connection=[[MCController sharedChannelController] connection];

    for (int i=0; i<[[connection.sessionBrowse connectedPeers] count]; i++) {
        [confirmSending addButtonWithTitle:[[[connection.sessionBrowse connectedPeers] objectAtIndex:i] displayName]];
    }
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
  MCConnection *connection=[[MCController sharedChannelController] connection];
    ...
    if (buttonIndex != [[connection.sessionBrowse connectedPeers] count]) {
        NSString *filePath = [_documentsDirectory stringByAppendingPathComponent:_selectedFile];
      MCConnection *connection=[[MCController sharedChannelController] connection];
      self.filenameCounter++;
      
        NSString *modifiedName = [NSString stringWithFormat:@"%@_ %@_", @"FileReceived", _selectedFile];
        NSURL *resourceURL = [NSURL fileURLWithPath:filePath];
        
        dispatch_async(dispatch_get_main_queue(), ^{
            NSProgress *progress = [connection.sessionBrowse sendResourceAtURL:resourceURL
                                                                            withName:modifiedName
                                                                              toPeer:[[connection.sessionBrowse connectedPeers] objectAtIndex:buttonIndex]
